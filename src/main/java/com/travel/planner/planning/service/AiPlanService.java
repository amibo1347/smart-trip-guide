...
public class AiPlanService {
    ...
    private String buildPrompt(Trip trip, long days, GeneratePlanRequest req, List<TripBooking> bookings) {
        ...
        return """
                ...
                - MEAL(식사)/SPOT(명소)/ACTIVITY 는 '실제 존재하는, 지도에서 한 곳으로 특정되는 고유 상호명'을
                  title 과 placeName 에 넣어라. '온천', '스파', '카페', '쇼핑', '시장' 같은 일반 범주어만 쓰지 마라.
                  반드시 그 도시에 실재하는 특정 업소의 고유 이름으로 좁혀라.
                  나쁜 예: "점심 식사", "온천", "스파", "쇼핑" /
                  좋은 예: "이치란 라멘 도톤보리점에서 점심", "오사카 스파월드(스파스미나미)", "고베 아리마온천 킨노유".
                  placeName 에는 구글 지도에서 바로 검색되는 정확한 고유 상호명(가능하면 지점명 포함),
                  address 에는 대략 위치(동/구/거리)를 넣어라.
                - 추천 우선순위: 같은 범주(예: 라멘집·온천)라면 '구글 지도 평점이 높고(대략 4.0 이상) 리뷰 수가 많은'
                  검증된 인기 장소를 우선 골라라. 평점/리뷰가 빈약하거나 폐업·불확실한 곳, 실재가 의심스러운 곳은 피하라.
                  단, 평점·인기의 기준은 '전 세계'가 아니라 '한국인 여행자'다. 한국인 관광객에게 실제로 인기 있고
                  후기가 많은 곳, 한국 여행 블로그·커뮤니티에서 자주 추천되는 검증된 대표 맛집/명소를 우선 추천하라.
